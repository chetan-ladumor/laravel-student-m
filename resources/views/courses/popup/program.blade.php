<div class="modal" id="program-show" tabindex="-1" role="dialog">
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title">Program</h5>
				<button type="button" class="close" data-dismiss="modal" aria-label="Close">
					<span aria-hidden="true">&times;</span>
				</button>
			</div>
			<div class="modal-body">
				<div class="row">
					<div class="col-sm-12">
						<input type="text" name="program" id="program" class="form-control" placeholder="Program">
					</div>
					<br>
					<div class="col-sm-12">
						<input type="text" name="program_description" id="program_description" class="form-control" placeholder="Program Description">
					</div>
				</div>
			</div>
			<div class="modal-footer">
				<button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
				<button type="button" class="btn btn-success btn-save-program">Save</button>
			</div>
		</div>
	</div>
</div>